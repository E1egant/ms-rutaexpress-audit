...
class AuditSecurityTest {
    ...
    void conRolAdminRetorna200() throws Exception {
        mvc.perform(get("/api/audit").with(jwt().authorities(new SimpleGrantedAuthority("ROLE_Admin"))))
                .andExpect(status().isOk());
    }

    @Test
    void conRolSinPermisoRetorna403() throws Exception {
        mvc.perform(get("/api/audit").with(jwt().authorities(new SimpleGrantedAuthority("ROLE_Operador"))))
